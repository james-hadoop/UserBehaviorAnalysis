...
object AdStatisticsByGeo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val adEventStream = env.readTextFile(resource.getPath).map(data => {
    ...
    val filterBlackListStream = adEventStream.keyBy(data => (data.userId, data.adId)).process(new FilterBlackListUser(100))

    //根据省份做分组，开窗聚合
    val adCountStream = adEventStream.keyBy(_.province).timeWindow(Time.hours(1), Time.seconds(5)).aggregate(new AdCountAgg(), new AdCountResult())
